...
export sim_python=/mnt/cpfs/guchenyang/miniconda3/envs/simpler_env/bin/python
export SimplerEnv_PATH=/mnt/cpfs/guchenyang/Code/SimplerEnv-OpenVLA
...
if [ -n "$1" ]; then
  ...
  MODEL_PATH=/mnt/cpfs/guchenyang/Code/starVLA/playground/SimplerEnv/0202_PaligemmaPI_bridgev2_chunk_16/checkpoints/steps_70000_pytorch_model.pt
fi

port=${2:-6678} # connect to your policy server port

# Video save directory name (3rd parameter)
# If not specified, use checkpoint name + timestamp
if [ -n "$3" ]; then
  video_dir_name="$3"
else
  video_dir_name="0202_Paligemma_PI_Bridge_16/test"
fi
